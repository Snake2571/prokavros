...
public class ClientRequest extends StringRequest {
    ...
    public static int CODE_ERROR_PHONE_OF_OTHER_CLIENT = 2;
    ...
    public static ClientRequest requestPostClient(final Client client, final PostResponseListener listener) {

        Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
        final String strJsonClient = gson.toJson(client);


        Log.d(TAG, "Json post: " + strJsonClient);
    ...
    public static ClientRequest requestPutClient(Client client,  final PostResponseListener listener) {
        ...
        Log.d(TAG, "Json put: " + strJsonClient);

        String url = ConstantsBikeRentalApp.URL_SERVER+"/"+"client/"+client.serverId;

        Map<String, String> params = new HashMap<>();
        params.put(PARAM_JSON_CLIENT, strJsonClient);

        return new ClientRequest(Request.Method.PUT, url, params, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                listener.onResponse(response);
            }
